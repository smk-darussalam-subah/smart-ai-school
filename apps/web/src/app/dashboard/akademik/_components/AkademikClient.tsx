...
interface Props { grades: Grade[]; attendances: Attendance[]; classes: { id: string; name: string }[]; assignments: { id: string; subject: string }[]; canManage: boolean; }
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2 border-b pb-2">
        {(['grades', 'attendance'] as const).map(t => (
          <Button key={t} variant={tab === t ? 'default' : 'ghost'} size="sm" onClick={() => setTab(t)} className={tab === t ? 'bg-smk-blue hover:bg-primary-700' : ''}>{t === 'grades' ? '📝 Nilai' : '📅 Absensi'}</Button>
        ))}
        {canManage && <div className="ml-auto"><Button size="sm" className="bg-smk-blue hover:bg-primary-700" onClick={() => tab === 'grades' ? setGradeOpen(true) : setAttendanceOpen(true)}>+ {tab === 'grades' ? 'Input Nilai' : 'Input Absensi'}</Button></div>}
      </div>
