...
function SummaryRail({ snapshot }: { snapshot: MonitoringSnapshot }) {
  ...
  return (
    <section aria-label="Ringkasan hari sekolah" className="overflow-x-auto rounded-lg border border-slate-200 bg-white">
      <div className="grid min-w-[44rem] grid-cols-[1.25fr_repeat(5,1fr)] divide-x divide-slate-200">
        <div className="px-4 py-3">
          ...
          <p className="mt-1 font-semibold text-slate-900">{snapshot.currentSegment ?? 'Di luar jam pelajaran'}</p>
        </div>
...
export default function MonitoringClient({
  ...
  async function confirmRevoke() {
    ...
    setMutationError(null);
    const result = confirmTarget === 'ALL'
    ...
    return (
      <section className="rounded-lg border border-amber-200 bg-amber-50 p-6" role="alert">
        <div className="flex gap-3">
          ...
          <div><h1 className="text-lg font-bold text-amber-950">Monitoring tidak tersedia</h1><p className="mt-1 text-sm text-amber-900">{initialError}</p></div>
        </div>
      </section>
    );
  }
    ...
    <div className="space-y-4">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">Status sesi authoritative, alert, dan kesehatan display terpasang.</p>
        </div>
      ...
      {initialError && (
        <section className="rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-900" role="alert">
          <div className="flex items-start gap-3"><AlertTriangle className="mt-0.5 h-5 w-5 shrink-0" /><div><p className="font-semibold">Data monitoring gagal dimuat</p><p>{initialError}</p><Button variant="outline" className="mt-3 min-h-11 border-red-300 bg-white" onClick={refresh}>Coba lagi</Button></div></div>
        </section>
      )}
      ...
      {stale && (
        <div className="flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900" role="status">
          <Clock3 className="h-5 w-5" /> Data terlambat diperbarui. Tampilan akan menyambung ulang otomatis; gunakan Perbarui bila diperlukan.
        </div>
      )}

      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_22rem]">
        <section className="min-w-0 rounded-lg border border-slate-200 bg-white" aria-labelledby="session-heading">
          <div className="border-b border-slate-200 p-4">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
              <div><h2 id="session-heading" className="font-bold text-slate-950">Sesi kelas</h2><p className="text-sm text-slate-500">{visibleSessions.length} sesi sesuai filter</p></div>
              <div className="grid gap-2 sm:grid-cols-[minmax(12rem,1fr)_10rem_auto]">
                <label className="relative"><span className="sr-only">Cari sesi</span><Search className="pointer-events-none absolute left-3 top-3 h-4 w-4 text-slate-400" /><Input className="h-11 pl-9" value={filters.query} onChange={(event) => setFilters((value) => ({ ...value, query: event.target.value }))} placeholder="Kelas, mapel, ruang, guru" /></label>
                <label><span className="sr-only">Filter status</span><select className="h-11 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm" value={filters.status} onChange={(event) => setFilters((value) => ({ ...value, status: event.target.value as MonitoringFilters['status'] }))}><option value="ALL">Semua status</option>{Object.entries(STATUS_LABEL).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
                <label className="flex min-h-11 items-center gap-2 rounded-lg border border-slate-200 px-3 text-sm"><input type="checkbox" checked={filters.attentionOnly} onChange={(event) => setFilters((value) => ({ ...value, attentionOnly: event.target.checked }))} /> Perlu tindakan</label>
              </div>
            </div>
          </div>
          <div className="divide-y divide-slate-200">
            ...
            {!initialError && visibleSessions.length === 0 && (
              <div className="p-10 text-center"><Clipboard className="mx-auto h-8 w-8 text-slate-400" /><p className="mt-3 font-semibold text-slate-800">Tidak ada sesi sesuai filter</p><p className="mt-1 text-sm text-slate-500">Ubah filter atau tunggu materialisasi jadwal berikutnya.</p></div>
            )}
          </div>
        </section>

        <aside className="space-y-4">
          <section className="rounded-lg border border-slate-200 bg-white" aria-labelledby="alert-heading">
            <div className="border-b border-slate-200 p-4"><h2 id="alert-heading" className="font-bold text-slate-950">Alert aktif</h2><p className="text-sm text-slate-500">Visual queue untuk tindak lanjut</p></div>
            <div className="divide-y divide-slate-200">
              {(initialSnapshot?.alerts ?? []).filter((alert) => !alert.acknowledged).map((alert) => (
                <div key={alert.id} className="p-4"><div className="flex items-start gap-3"><AlertTriangle className={`mt-0.5 h-5 w-5 shrink-0 ${alert.severity === 'CRITICAL' ? 'text-red-700' : 'text-amber-700'}`} /><div><p className="font-semibold text-slate-900">{alert.className}</p><p className="text-sm text-slate-600">{alert.room ?? 'Ruang belum ditetapkan'} · {alert.stage}</p></div></div></div>
              ))}
              {!initialSnapshot?.alerts.some((alert) => !alert.acknowledged) && <p className="p-4 text-sm text-slate-500">Tidak ada alert aktif.</p>}
            </div>
          </section>
