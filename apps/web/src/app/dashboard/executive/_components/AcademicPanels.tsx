...
export function GradeBoxPlot({ grades }: { grades: GradeAnalytics | null }) {
  ...
  return (
    <Card
      title="Distribusi Nilai per Jurusan"
      subtitle="Median, kuartil & sebaran"
      icon={BarChart3}
      level="real"
          ...
          {[100, 85, 70, 55].map((g) => (
            <g key={g}>
              ...
              <text
                ...
                y={y(g) + 4}
                textAnchor="end"
                className="fill-[#6b8079] text-[10px] font-semibold"
              >
                {g}
              </text>
            </g>
            ...
            return (
              <g key={m.majorCode}>
                ...
                <rect
                  ...
                  y={y(m.q3)}
                  width={32}
                  height={Math.max(2, y(m.q1) - y(m.q3))}
                  rx={4}
                  fill="#a7f3d0"
                  stroke="#047857"
                  strokeWidth={1.4}
...
export function ScatterCorrelation({ grades }: { grades: GradeAnalytics | null }) {
  ...
  if (pts.length >= 2) {
    ...
    line = {
      x1: xv(60),
      y1: yv(slope * 60 + intercept),
      x2: xv(100),
      y2: yv(slope * 100 + intercept),
    };
  }
  return (
    <Card
      title="Korelasi Kehadiran ↔ Nilai"
      subtitle={`r = ${r} · kehadiran prediktor prestasi`}
      icon={Activity}
      level="real"
    ...
    >
      {pts.length === 0 ? (
        ...
        <svg viewBox={`0 0 ${W} ${H}`} className="w-full">
          ...
          <text x={40} y={H - 16} className="fill-[#6b8079] text-[10px] font-semibold">
            60%
          </text>
          <text
            x={W - 8}
          ...
          {pts.map((p, i) => {
            return (
              <circle key={i} cx={xv(p.x)} cy={yv(p.y)} r={3.6} fill="#10b981" opacity={0.8}>
                <title>{`Kehadiran ${p.x}%, Nilai ${p.y}`}</title>
              </circle>
            );
          })}
        </svg>
...
// ── Heatmap ketuntasan KKTP authoritative (jurusan × mapel) ──────────────────
export function KkmHeatmap({ grades }: { grades: GradeAnalytics | null }) {
  ...
  return (
    <Card
      title="Ketuntasan KKTP"
      subtitle="Ambang per mapel · jurusan × mapel"
      icon={ClipboardList}
      level="real"
        ...
        <>
                ...
                <tr key={m}>
                  ...
                  {matrix.subjects.map((s) => {
                    ...
                    return (
                      <td
                        key={s}
                        className="rounded-md px-1 py-2 text-center text-[10.5px] font-bold text-white"
                        style={{ background: passColor(p) }}
                        aria-label={`${m} ${s}: ${p === null ? 'KKTP belum tersedia atau tidak ada data' : `${Math.round(p)}% tuntas`}`}
                      >
                        {p === null ? '–' : Math.round(p)}
                      </td>
                    );
          ...
          <div className="mt-3 flex flex-wrap gap-2 text-[10px] font-semibold text-[#6b8079]">
            <span className="inline-flex items-center gap-1">
              <i className="h-2.5 w-2.5 rounded-sm" style={{ background: '#e11d48' }} />
              &lt;60
            </span>
            <span className="inline-flex items-center gap-1">
              <i className="h-2.5 w-2.5 rounded-sm" style={{ background: '#d97706' }} />
              60–69
            </span>
            <span className="inline-flex items-center gap-1">
              <i className="h-2.5 w-2.5 rounded-sm" style={{ background: '#f59e0b' }} />
              70–79
            </span>
            <span className="inline-flex items-center gap-1">
              <i className="h-2.5 w-2.5 rounded-sm" style={{ background: '#10b981' }} />
              80–89
            </span>
            <span className="inline-flex items-center gap-1">
              <i className="h-2.5 w-2.5 rounded-sm" style={{ background: '#059669' }} />
              90+
            </span>
          </div>
          <p className="mt-2 text-[10px] leading-4 text-[#6b8079]">
            Persentase dihitung terhadap KKTP authoritative masing-masing mapel; sel abu-abu tidak
            diasumsikan memakai nilai 75.
          </p>
        </>
      )}
    </Card>
