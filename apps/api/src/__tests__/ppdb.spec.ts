...
describe('SubmitLeadSchema — hardening validasi + normalisasi phone', () => {
  const { SubmitLeadSchema: schema } = jest.requireActual('../ppdb/dto/submit-lead.dto');

  // ── Normalisasi phone (format yang umum diketik calon siswa) ────────────────
  ...
  it('phone format 08xxx (lokal) → dinormalisasi ke 62xxx dan lolos', () => {
    ...
    // Verifikasi nilai tersimpan sudah ternormalisasi
  ...
  it('phone terlalu pendek setelah normalisasi → gagal refine', () => {
    const result = schema.safeParse({ ...VALID_SUBMIT_DTO, phone: '0812' }); // terlalu pendek
