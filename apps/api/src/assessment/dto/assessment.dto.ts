...
export const CreateAssessmentSessionSchema = z
  .object({
  ...
  .superRefine((value, ctx) => {
    if (value.type === 'diagnostik' && value.gradeTarget != null) {
      ctx.addIssue({
        ...
        path: ['gradeTarget'],
        message: 'Diagnostik tidak membuat Grade',
      });
    }
    if (value.type === 'formatif' && value.gradeTarget != null && value.gradeTarget !== 'uh') {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['gradeTarget'],
        message: 'Formatif harus menargetkan UH',
      });
    }
    if (value.type === 'sumatif' && value.gradeTarget !== 'uts' && value.gradeTarget !== 'uas') {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['gradeTarget'],
        message: 'Sumatif wajib memilih UTS atau UAS',
      });
    }
  });
export type CreateAssessmentSessionDto = z.infer<typeof CreateAssessmentSessionSchema>;

export const UpdateAssessmentSessionSchema = z
  .object({
...
export const ListAssessmentSessionSchema = z
  .object({
...
export const FamilyRemedialQuerySchema = z
  .object({
...
export const RemedialCandidatesQuerySchema = z
  .object({
...
export const CreateRemedialSessionSchema = z
  .object({
    title: z.string().trim().min(3).max(255),
...
export const UpdateRemedialSessionSchema = z
  .object({
    title: z.string().trim().min(3).max(255).optional(),
    questionSelections: QuestionSelectionListSchema.optional(),
...
export const CancelRemedialSessionSchema = z
  .object({
...
export const FinalizeRemedialParticipantSchema = z
  .object({
...
export const RetryRemedialParticipantSchema = z
  .object({
    participantId: z.string().uuid(),
    title: z.string().trim().min(3).max(255).optional(),
    questionSelections: QuestionSelectionListSchema,
...
export const SubmitResponseSchema = z
  ...
  .strict();
type ParsedSubmitResponseDto = z.infer<typeof SubmitResponseSchema>;
export type SubmitResponseDto = Omit<ParsedSubmitResponseDto, 'signals'> & {
  signals?: ParsedSubmitResponseDto['signals'];
};

export const ReviewLateSubmissionSchema = z
  .object({
    decision: z.enum(['accept', 'reject']),
    note: z.string().trim().min(3).max(500),
  })
  .strict();
export type ReviewLateSubmissionDto = z.infer<typeof ReviewLateSubmissionSchema>;
...
export const AssessmentRuntimeSignalSchema = z
  .object({
    eventId: z.string().uuid(),
    type: z.enum([
      'heartbeat',
  ...
  .strict();
export type AssessmentRuntimeSignalDto = z.infer<typeof AssessmentRuntimeSignalSchema>;
