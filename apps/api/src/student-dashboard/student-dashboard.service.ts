      ...
      students.map(async (s) => {
        ...
        const sessionVisibility: Prisma.AssessmentSessionWhereInput[] = [
          ...(s.classId ? [{ purpose: 'regular' as const, classId: s.classId }] : []),
          {
            ...
            remedialParticipants: {
              some: { studentId: s.id, status: { not: 'cancelled' as const } },
            },
          },
        ];
        ...
        const sessionAssignments = await Promise.all(
          sessions.map(async (sess) => {
            ...
            const status =
              !resp || !resp.submittedAt ? 'pending' : resp.score !== null ? 'graded' : 'submitted';
            const subject = sess.module?.subject ?? sess.teachingAssignment?.subject ?? null;
            const participant = sess.remedialParticipants[0];
            const exposeOwnRemedialLifecycle =
              user.roles.includes('SISWA') && sess.purpose === 'remedial' && Boolean(participant);
            const kktp = await resolveKktpThreshold(
              this.prisma,
              sess.purpose === 'remedial' && participant
                ? {
                    participantSnapshot: {
                      value: participant.kktpValue,
                      provenance: participant.kktpProvenance,
                    },
                  }
                : {
            ...
            return {
              ...
              guru: null,
              status,
              lateSubmissionStatus: resp?.lateSubmissionStatus ?? null,
    ...
    const data = await Promise.all(
      students.map(async (s) => {
        ...
        const bySubject = new Map<
          string,
          NaComponents & { subject: string; academicYear: string; semester: number }
        >();
        for (const g of grades) {
          const subject = g.assignment.subject;
          const key = `${subject}\u0000${g.academicYear}\u0000${g.semester}`;
          const comp = bySubject.get(key) ?? {
            subject,
        ...
        const subjects = await Promise.all(
          [...bySubject.values()].map(async (comp) => {
            const na = naOf(comp);
